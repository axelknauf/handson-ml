...
# ==== 1) fetch and extract source data
fetch_housing_data()
housing = load_housing_data()

# ==== 2) create test and training sets using derived column
# 2.1) Create column for income categories
housing["income_cat"] = np.ceil(housing["median_income"] / 1.5)
housing["income_cat"].where(housing["income_cat"] < 5, 5.0, inplace=True)

# 2.2) Create test and trainings sets using this new column
split = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
for train_index, test_index in split.split(housing, housing["income_cat"]):
    strat_train_set = housing.loc[train_index]
    strat_test_set = housing.loc[test_index]

# 2.3) remove temporary column again from sets 
for set in (strat_test_set, strat_train_set):
    set.drop(["income_cat"], axis=1, inplace=True)

# 2.4) Make sure we are using only the training set for our models
housing = strat_train_set.copy()

# ==== 3) add new derived columns on training set "housing"
housing["rooms_per_household"] = housing["total_rooms"] / housing["households"]
housing["bedrooms_per_room"] = housing["total_bedrooms"] / housing["total_rooms"]
housing["population_per_household"] = housing["population"] / housing["households"]

# FIXME does this make sense / work at all?
housing = strat_train_set.drop("median_house_value", axis=1)
housing_labels = strat_train_set["median_house_value"].copy()

# ==== 4) Set up processing pipeline
# Recap:
...
# ==== 10) Use grid search to find best parameters
param_grid = [
...
# ==== 11) Use best parameters / model to verify against the test set
